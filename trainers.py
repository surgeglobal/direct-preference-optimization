...
class BasicTrainer(object):
    ...
    def forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]]) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        """
        chosen_logits = model(batch['prompt_input_ids'], attention_mask=batch['prompt_attention_mask']).logits.to(torch.float32)
        chosen_logps = _get_batch_logps(chosen_logits, batch['chosen_input_ids'], average_log_prob=False)

        rejected_logits = model(batch['prompt_input_ids'], attention_mask=batch['prompt_attention_mask']).logits.to(torch.float32)
